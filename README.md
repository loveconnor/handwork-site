...
Benchmark values are the README's September 2026 project-maintained pilots: corrected pagination, original interleaved async-search series, and authorization/cache isolation. The task selector keeps these distinct. These are frozen-build results, not fresh measurements or claims about current releases. Source links, machine, model, sample counts, and caveats are displayed with the table. Update `public/app.js` and the no-JavaScript default table in `src/index.html` together when updating data.
